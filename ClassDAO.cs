...
namespace HW310319_CountryDataBase
{
    class ClassDAO : IClassDAO
    {
        ...
        public List<object> AllCountriesFromFirstLetter(string FirstLetter)
        {
            ...
            using (SQLiteCommand cmd = new SQLiteCommand($"SELECT * FROM COUNTRY JOIN CoupitalCity ON COUNTRY.CAPITALCITY_ID == CoupitalCity.ID WHERE COUNTRY.NAME == '{FirstLetter}%'", connection))
            {
                using (SQLiteDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        Country CurrentCountry = new Country
                        {
                            ...
                            CapitalCityId = (int)reader["CAPITACITY_ID"]
                        };
